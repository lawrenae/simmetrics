...
//I thought it would be cool to have a kind-of overloaded () operater effect going on.
function Soundex(input) {
	if(this instanceof Soundex) {
		if(input != null) {
			if(input._isSimMetric) {
				this.iSM = input;
			} else {
				return this.soundex(input);
			}
		} else {
			this.iSM = new JaroWinkler();
		}
	} else {
		return (new Soundex()).soundex(input);
	}
}
